...
public class ImageManager {
    ...
    */
    ...
    public static IImageList makeImageList(Context context, ContentResolver cr,
        ...
        // @eaceto
        /// include a new list type "CameraActionList" with 2 images, camera and video

        l.add(new CameraActionList(context,cr, null, sort, bucketId));
    ...
    public static IImageList makeImageList(Context context, ContentResolver cr, Uri uri,
            int sort) {
        ...
        if (uriString.startsWith("content://drm")) {
            return makeImageList(context,cr, DataLocation.ALL, INCLUDE_DRM_IMAGES, sort,
                    null);
        } else if (uriString.startsWith("content://media/external/video")) {
            return makeImageList(context,cr, DataLocation.EXTERNAL, INCLUDE_VIDEOS,
                    sort, null);
        } else if (isSingleImageMode(uriString)) {
            return makeSingleImageList(context, cr, uri);
        } else {
            String bucketId = uri.getQueryParameter("bucketId");
            return makeImageList(context,cr, DataLocation.ALL, INCLUDE_IMAGES, sort,
                    bucketId);
        }
    }
    ...
    public static IImageList makeImageList(Context context, ContentResolver cr,
        ...
        ImageListParam param = getImageListParam(location, inclusion, sort,
                bucketId);
        return makeImageList(context, cr, param);
    }

    public static IImageList makeEmptyImageList(Context context) {
        return makeImageList(context, null, getEmptyImageListParam());
    }

    public static IImageList  makeSingleImageList(Context context, ContentResolver cr, Uri uri) {
        return makeImageList(context, cr, getSingleImageListParam(uri));
    }
