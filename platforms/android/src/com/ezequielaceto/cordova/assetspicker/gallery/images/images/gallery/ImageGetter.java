...
public class ImageGetter {
    ...
    private class ImageGetterRunnable implements Runnable {
        ...
        private void executeRequest() {
            ...
            for (int anOrder1 : order) {
                if (mCancel) return;
                int imageNumber = mCurrentPosition + anOrder1;
                if (imageNumber >= 0 && imageNumber < imageCount) {
                    if (!mCB.wantsThumbnail(mCurrentPosition, anOrder1)) {
                        continue;
                    }
                    ...
                    Runnable cb = callback(mCurrentPosition, anOrder1,
                            true,
            ...
            for (int anOrder : order) {
                if (mCancel) return;
                int imageNumber = mCurrentPosition + anOrder;
                if (imageNumber >= 0 && imageNumber < imageCount) {
                    if (!mCB.wantsFullImage(mCurrentPosition, anOrder)) {
                        continue;
                    }
                    ...
                    Runnable cb = callback(mCurrentPosition, anOrder,
